...
namespace assignment {
  ...
  int linear_search(const std::vector<int>& arr, int index) {
    ...
    for (int curr_pos = 0; curr_pos-index<0; curr_pos++) {
      if (arr[curr_pos] == arr[index] || arr[curr_pos] > arr[index]){
        return curr_pos;
      }
  ...
  int binary_search(const std::vector<int>& arr, int index) {
    ...
    int stop = index /* здесь что-то не так ... */;
    ...
    while (start <= stop) {
      ...
      if (arr[index] == arr[middle]) {
        ...
        stop = middle-1;/* здесь что-то не так ... */
      }
      else{
        start = middle+1;
      }
